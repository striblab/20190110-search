...
import nilfMN from '../sources/ranks.json';
    ...
    render() {
        ...
        g.append("g")
            ...
            .style("fill", function(d) {
                ...
                for (var i=0; i < dataMN.length; i++) {
                    ...
                    if (dataMN[i].Geography == d.properties.COUNTYNAME) {
                        var color_scale = d3.scaleLinear().domain([0,1,2,3,4,5,6,7,8]).range(['#b37862', '#80ADAD', '#8CBF82', '#7D739C', '#aaaaaa', '#636363', '#DEA381', '#9E403C']);
                        return color_scale(dataMN[i].index);
                    }
                }
            ...
            .call(tooltip(function(d, i) {
                ...
                for (var i = 0; i < dataMN.length; i++) {
                    if (dataMN[i].Geography == d.properties.COUNTYNAME) {
                        var index = dataMN[i].index;
                        var color_scale = d3.scaleLinear().domain([0,1,2,3,4,5,6,7,8]).range(['#b37862', '#80ADAD', '#8CBF82', '#7D739C', '#aaaaaa', '#636363', '#DEA381', '#9E403C']);
                        var color = "#ffffff";
                        return "<div class='countyName'>" + d.properties.COUNTYNAME + " County</div><div class='number'><span class='legendary' style='color:" + color + "; background-color:" + color_scale(index) + ";'>" + dataMN[i].candidate + "</div>";
                    }
                }
              }));
